...
agg_daily_wd<-paste0(mainDir,"/rainfall/working_data/hi_mesonet/uh_meso")
urlRFday<-"https://ikeauth.its.hawaii.edu/files/v2/download/public/system/ikewai-annotated-data/HCDP/workflow_data/preliminary/rainfall/working_data/hi_mesonet/uh_meso/"
...
meso_filename<-paste0(format((currentDate),"%Y%m%d"),"_hi_mesonet_parsed.csv") #dynamic file name that includes date
all_meso<-read.csv(paste0(ikeUrl,"/data_aqs/data_outputs/hi_mesonet/parse/",meso_filename))
head(all_meso)
#unique(all_meso$var)

#subset precip var, convert inch to mm and convert UTC to HST
all_meso_rf<-subset(all_meso,variable=="RF_1_Tot300s")# subset precip only
...
all_meso_rf$timestamp<-strptime(all_meso_rf$timestamp, format="%Y-%m-%d %H:%M:%S", tz="UTC")
attr(all_meso_rf$timestamp,"tzone") <- "Pacific/Honolulu" #convert TZ attribute to HST
all_meso_rf$timestamp<-(all_meso_rf$timestamp-36000) #minus 10 hrs for UTC to HST
all_meso_rf$timestamp<-(all_meso_rf$timestamp)-1 #minus 1 second to put midnight obs in last day
head(all_meso_rf)
tail(all_meso_rf)
...
stations<-unique(all_meso_rf$nws_id)

#start daily RF loop
print("daily rf loop started...")
# j<-stations[1]
for(j in stations){
  sta_data<-subset(all_meso_rf,nws_id==j)
  sta_data_xts<-xts(sta_data$value,order.by=sta_data$timestamp,unique = TRUE) #make xtended timeseries object
...
#head(meso_daily_rf_today_final)
#tail(meso_daily_rf_today_final)

#write or append daily rf data monthly file
setwd(agg_daily_wd)#server path daily agg file
rf_month_filename<-paste0(format((currentDate),"%Y_%m"),"_HIMeso_daily_rf.csv") #dynamic file name that includes month year so when month is done new file is written
system(paste("wget -N -P",agg_daily_wd,paste0(urlRFday,rf_month_filename))) #fetch uh hiMeso daily rf month file from server
